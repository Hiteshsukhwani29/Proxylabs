    ...
    <>
      <div className="flex items-start pt-6 px-4 flex-0">
        ...
        <div className="flex justify-around flex-1">
          <div className={`flex-0 flex items-center ${AlreadyLoggedIn?'block':'hidden'}`}>
          ...
          {!AlreadyLoggedIn ? <div className="bg-white flex h-min w-max px-6 py-1 rounded-full flex-0 justify-center cursor-pointer" onClick={() => setShowModal(true)}>
            <div>Sign Up</div>
            <img className="h-6 w-6 ml-2" src={OutlinePerson} />
          </div>:<div className="bg-white flex h-min w-max px-6 py-1 rounded-full flex-0 justify-center cursor-pointer">
